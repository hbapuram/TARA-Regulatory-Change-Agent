...
class AgentInvestigationRequest(RunRequest):
    """A bounded request for the live case-investigator narrative."""


class AgentInterviewRequest(RunRequest):
    """A request to explain one already-determined missing fact.

    The browser supplies only a question ID that came from the current
    deterministic run. The endpoint re-runs the controls before calling the
    model, so the model cannot introduce a new question or alter its meaning.
    """
...
def _agent_trace(result: Any) -> list[dict[str, Any]]:
    """Return the browser-safe subset of an LLM MCP trace."""
    return [
        {
            "tool": call.name,
            "arguments": call.arguments,
...
def _safe_interview_summary(answer: str, canonical_question: str) -> str:
    ...
    A model may clarify why a human has to supply a fact, but it must not
    restate a rule outcome, source, number, or case-specific conclusion. A
    nonconforming response is withheld rather than repaired or paraphrased.
    """
    text = " ".join(answer.replace("\r", " ").split()).lstrip("-• ")
    question = " ".join(canonical_question.split())
    if not text.startswith(question):
        raise HTTPException(status_code=502, detail="Live interview did not preserve the canonical question exactly.")
    explanation = text[len(question):].strip()
    if not explanation or len(explanation.split()) > 42:
        raise HTTPException(status_code=502, detail="Live interview did not return a concise explanation.")
    forbidden = (
        r"\b(?:compass|course|survey|anchor|atlas|almanac|meridian|plot|"
        r"confirmed|exempt|indeterminate|applies|does not apply|obligation|"
        r"source(?:_id)?|provision|domain(?:_id)?|holding(?:_id)?|"
        r"section\s+\d|\d+(?:\.\d+)?%|€|\$|rate|tax|CGT|"
        r"yes|no|relief|apply|dismiss|eligib\w*|classif\w*|trigger\w*|"
        r"occupancy|property|residence)\b"
    )
    if re.search(forbidden, explanation, flags=re.IGNORECASE):
        raise HTTPException(status_code=502, detail="Live interview returned restricted operational detail.")
    return f"{question}\n{explanation}"


def _orchestrate_case_investigator(req: RunRequest) -> dict[str, Any]:
    """Run a second, visible agent investigation without changing controls.

    The existing /api/ai/run route runs an orchestration pass as part of the
    default walkthrough. This separate, user-invoked pass makes the agent's
    ...
    with TemporaryDirectory() as tmp:
        ...
        register_path = tmp_path / "register.json"
        register_path.write_text(json.dumps(register, indent=2), encoding="utf-8")
        prompt = f"""You are TARA's live case investigator for a prepared, synthetic demonstration.

Investigate the regulatory change and its relevance to this one prepared case. The event date is {as_of}. The supplied applicability answers are {json.dumps(req.answers, sort_keys=True)}.

First call list_domains and list_holdings. Then identify the relevant domain, call list_sources for it, and use only IDs returned by those discovery tools. You must call survey_detect_change, compass_assess, and course_plan before answering.

Return exactly three concise, plain-language bullets:
1. What changed, with the provision from the tool result.
2. What the deterministic controls found for this case, or the one missing fact that prevents a conclusion.
3. What a qualified human reviewer should check next.

Never calculate, select a rate, create an action, or approve evidence yourself. Never invent names, dates, source IDs, rates, obligations, or conclusions. Your explanation is a live investigation narrative only; the separately rendered deterministic controls remain authoritative."""
        required = {"list_domains", "list_holdings", "list_sources", "survey_detect_change", "compass_assess", "course_plan"}
        try:
            result = llm.run(
                prompt,
            ...
            raise HTTPException(status_code=502, detail=f"Live agent investigation did not complete: {exc}") from exc

    trace = _agent_trace(result)
    observed = {step["tool"] for step in trace}
    if not required <= observed:
...
def _orchestrate_missing_fact_interview(req: RunRequest, question: dict[str, Any]) -> dict[str, Any]:
    ...
    with TemporaryDirectory() as tmp:
        ...
        register_path = tmp_path / "register.json"
        register_path.write_text(json.dumps(register, indent=2), encoding="utf-8")
        prompt = f"""You are TARA's live interview guide for a prepared, synthetic demonstration.

The deterministic controls have paused one rule because a material fact is missing. The canonical question is exactly:
{question['text']}

The question belongs to a prepared case. The supplied answers are {json.dumps(req.answers, sort_keys=True)}.

First call list_domains and list_holdings. Then call compass_assess for the prepared case using the supplied answers. Return exactly two concise sentences: first repeat the canonical question exactly; second, in no more than 22 words, explain only that a qualified reviewer must confirm the fact before TARA can re-run its deterministic controls.

Do not ask a different question. Do not suggest Yes or No. Do not mention a tool, result label, rule, source, identifier, date, amount, rate, jurisdiction, or legal conclusion. Do not calculate, decide applicability, create an action, or give legal, tax, immigration, or filing advice."""
        required = {"list_domains", "list_holdings", "compass_assess"}
        try:
            result = llm.run(
                prompt,
            ...
            raise HTTPException(status_code=502, detail=f"Live agent interview did not complete: {exc}") from exc

    trace = _agent_trace(result)
    observed = {step["tool"] for step in trace}
    if not required <= observed:
        raise HTTPException(status_code=502, detail="Live agent interview did not complete its required discovery steps.")
    try:
        summary = _safe_interview_summary(result.answer, question["text"])
        output_withheld = False
    except HTTPException:
        # The agent still performed the required guarded calls; only its
        # free-form prose is withheld. A predictable, non-decisional bridge
        # keeps the human confirmation path usable and makes the policy visible
        # to the browser without attempting to edit model output.
        summary = (
            f"{question['text']}\n"
            "The live investigator checked the prepared case. A qualified reviewer must confirm this fact before TARA re-runs its deterministic controls."
        )
...
def agent_investigate(req: AgentInvestigationRequest) -> dict[str, Any]:
    """Visible live investigation plus a fresh, authoritative control result."""
    investigator = _orchestrate_case_investigator(req)
    return {"investigator": investigator, "controls": _run_pipeline(req)}


@app.post("/api/agent/interview")
def agent_interview(req: AgentInterviewRequest) -> dict[str, Any]:
    """Explain exactly one deterministic missing-fact question before confirmation."""
    controls = _run_pipeline(req)
    question = next((q for q in controls["pending_questions"] if q["question_id"] == req.question_id), None)
    if question is None:
        raise HTTPException(
            status_code=409,
            detail="That question is not currently pending. Re-run the controls before requesting a live interview.",
        )
    interview = _orchestrate_missing_fact_interview(req, question)
    return {"question": question, "interview": interview}
