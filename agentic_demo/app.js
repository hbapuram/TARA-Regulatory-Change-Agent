...
const LS_KEY = "tara.agentic-demo.api";
...
async function doRun() {
  S.busy = true; S.error = null; S.orchestration = null; S.aiFallbackMessage = null;
  S.investigator = null; S.interviews = {}; S.pendingAnswer = null; S.agentError = null; render();
  try {
    if (S.mode === "live") {
      // Render the authoritative control result first. The live investigator is
      // explicit and user-invoked, so a slow model can never block the source,
      // applicability, action, or evidence workflow.
      S.run = await api("/api/run", runPayload());
      if (S.aiPreference === "llm" && !S.aiAvailable) {
        S.aiFallbackMessage = "Live agent investigation is not configured on this service. The deterministic controls remain available.";
      }
    } else {
...
function runPayload() {
  return { holder: S.holder, holdings: S.holdings, answers: S.answers, as_of: S.asOf };
}

async function doInvestigate() {
  if (!(S.mode === "live" && S.aiAvailable && S.aiPreference === "llm")) {
    S.agentError = "The live case investigator needs AI/MCP mode and an available connection. The deterministic controls remain available.";
    render();
    return;
  }
  ...
  try {
    ...
    S.investigator = out.investigator;
    // The live agent explanation is never the authoritative result. Refresh the
    // screen only from the separately returned deterministic controls.
    S.run = out.controls;
  } catch (e) {
    S.agentError = "The live case investigator did not complete: " + e.message;
  }
  S.agentBusy = false; render();
}
...
async function doAgentInterview(questionId) {
  ...
  S.agentBusy = true; S.agentError = null;
  S.interviews[questionId] = { busy: true };
  render();
  try {
    const out = await api("/api/agent/interview", Object.assign(runPayload(), { question_id: questionId }));
    S.interviews[questionId] = out.interview;
  } catch (e) {
    S.interviews[questionId] = { error: "The live interview guide did not complete: " + e.message };
  }
  S.agentBusy = false; render();
}

async function confirmAnswer(questionId, value) {
  S.answers[questionId] = value;
  S.pendingAnswer = null; S.cell = null; S.actionId = null; S.closure = null; S.verification = null;
  S.investigator = null; S.interviews = {}; S.agentError = null;
  await doRun();
}
...
function renderChrome() {
  ...
  $("#engineLabel").textContent =
    S.mode === "live" && S.aiPreference === "llm" && S.aiAvailable ? "live agent ready"
    : S.mode === "live" ? "controls ready"
    : S.mode === "replay" ? "backup replay" : S.mode === "checking" ? "connecting" : "offline";
  $("#engineEndpoint").textContent =
    S.mode === "live" ? `${S.aiPreference === "llm" && S.aiAvailable ? "agent + controls · " : "controls · "}${S.api.replace(/^https?:\/\//, "")}`
    : S.mode === "replay" ? "recorded " + (REPLAY ? REPLAY.captured_at : "") : "";
  const modeButton = $("#btnAiMode");
  if (modeButton) {
    modeButton.textContent = S.aiPreference === "llm" ? "Live agent: on" : "Live agent: off";
    modeButton.disabled = S.mode !== "live" && S.aiPreference === "llm";
    modeButton.title = S.aiPreference === "llm"
      ? "The live investigator is available. Click to disable live agent calls."
      : "Live agent calls are off. Click to return to AI/MCP mode.";
  }
...
function stageBrief() {
  const engineNote =
    S.mode === "live" && S.aiPreference === "llm" && S.aiAvailable
      ? `<div class="note"><b>Live agent investigation is ready.</b> Ask the investigator to discover TARA's MCP tools and explain the prepared case. Deterministic controls still validate the rule, date, calculation, and evidence.</div>`
      : S.mode === "live"
      ? `<div class="note"><b>Deterministic controls are ready.</b> This runs the controlled rule, date, calculation, and evidence checks without a live agent call.</div>`
      : S.mode === "replay"
  ...
  return `<header>
    ...
    <div class="stack">
      ...
      <div class="journey" aria-label="Demo journey">
        <div class="journey-step"><span>1</span><div><b>Controls find the facts</b><p>TARA checks the source and case against guarded rules.</p></div></div>
        <div class="journey-step"><span>2</span><div><b>Agent investigates live</b><p>The LLM discovers MCP tools and explains the route it took.</p></div></div>
        <div class="journey-step"><span>3</span><div><b>A human confirms proof</b><p>Watch 41% return, 38% pass, and the record remain inspectable.</p></div></div>
      </div>
...
function agentTraceHtml(trace) {
  const labels = {
    list_domains: "Mapped the available rule areas",
    list_sources: "Found the controlled source",
    list_holdings: "Located the prepared case item",
    survey_detect_change: "Compared source versions",
    compass_assess: "Checked applicability",
    plot_align: "Aligned requirements to the event date",
    course_plan: "Prepared the dated action plan",
    anchor_verify: "Checked submitted evidence",
    meridian_survey: "Checked linked jurisdictions",
    atlas_reconstruct: "Read the proof record"
  };
  return `<ol class="mcp-trace">${(trace || []).map((step, i) =>
    `<li><span>${i + 1}</span><b>${esc(labels[step.tool] || titleCase(step.tool))}</b><em>${esc(step.status || "completed")}</em></li>`
  ).join("")}</ol>`;
}
...
function stageChart() {
  ...
  const aiHeadline = S.orchestration
    ...
    : S.mode === "live" && S.aiPreference === "llm" && S.aiAvailable
    ? "TARA checked the change. A live agent can investigate it."
  ...
  const aiLede = S.orchestration
    ...
    : S.mode === "live" && S.aiPreference === "llm" && S.aiAvailable
    ? "The controls have verified the change below. Ask the live investigator to discover the guarded MCP tools and explain the same prepared case; the controls remain authoritative."
...
function stageInterview() {
  ...
  const asked = qs.length ? qs.map(q => {
    const selected = S.pendingAnswer && S.pendingAnswer.questionId === q.question_id ? S.pendingAnswer : null;
    const answerControls = selected ? `<div class="answer-confirm">
      <b>You selected “${selected.value ? "Yes" : "No"}”.</b>
      <p>This case fact is not updated until you confirm it. Confirmation re-runs TARA's deterministic checks; the live agent never supplies an answer.</p>
      <div class="row"><button class="btn" data-answer-confirm="${esc(q.question_id)}|${selected.value}" type="button">Confirm ${selected.value ? "Yes" : "No"}</button>
        <button class="btn ghost" data-answer-cancel type="button">Choose again</button></div>
    </div>` : `<div class="row">
...
function applyPreset(id) {
  ...
  S.orchestration = null; S.aiFallbackMessage = null; S.investigator = null;
  S.interviews = {}; S.pendingAnswer = null; S.agentBusy = false; S.agentError = null;
}
...
document.addEventListener("click", async ev => {
  const t = ev.target.closest("[data-leg],[data-goto],[data-preset],[data-multi],[data-del],[data-cell],[data-answer],[data-answer-confirm],[data-answer-cancel],[data-agent-interview],[data-variant],button");
  if (!t) return;
  ...
  if (t.id === "runInvestigator") { await doInvestigate(); return; }
  ...
  if (t.dataset.answer) {
    ...
    S.pendingAnswer = { questionId: qid, value: val === "true" };
    render();
    return;
  }
  if (t.dataset.answerConfirm) {
    const [qid, val] = t.dataset.answerConfirm.split("|");
    await confirmAnswer(qid, val === "true");
    return;
  }
  if (t.dataset.answerCancel !== undefined) {
    S.pendingAnswer = null;
    render();
    return;
  }
  if (t.dataset.agentInterview) {
    await doAgentInterview(t.dataset.agentInterview);
    return;
  }
